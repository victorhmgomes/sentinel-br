    ...
    out = {
        ...
        "indicators_doc": [
            {"id": "volume_zscore", "name": "Pico de volume (Z-score 30d)",
             "name_en": "Volume spike (30-day Z-score)",
             ...
             "desc_en": "Detects liquidity spikes on a pair/exchange by comparing daily volume against the 30-day rolling mean and standard deviation. Z>3 indicates a statistically significant outlier — the classic cash-out-via-crypto pattern.",
             "trigger": "Z ≥ 3 (high), Z ≥ 4 (critical)",
             "trigger_en": "Z ≥ 3 (high), Z ≥ 4 (critical)"},
            {"id": "stablecoin_flight", "name": "Fuga para stablecoin",
             "name_en": "Flight to stablecoin",
             ...
             "desc_en": "Ratio between USDT-BRL and BTC-BRL volume. Jumps indicate mass rotation into stablecoin — a common laundering technique at VASPs because it preserves value during layering.",
             "trigger": "Z ≥ 3 sobre janela 30d",
             "trigger_en": "Z ≥ 3 over a 30-day window"},
            {"id": "br_premium", "name": "Premium BR vs. mercado global",
             "name_en": "BR premium vs. global market",
             ...
             "desc_en": "Deviation between BTC-BRL price and the theoretical price (BTCUSDT × USDT-BRL). Abnormal premiums signal sudden domestic demand — frequently associated with post-incident cash-out.",
             "trigger": "|Z| ≥ 3 e desvio > 1%",
             "trigger_en": "|Z| ≥ 3 and deviation > 1%"},
            {"id": "price_volume_divergence", "name": "Volume sem movimento de preço",
             "name_en": "Volume without price movement",
             ...
             "desc_en": "Volume Z>2 with daily return < 0.5%. Suggests wash-trading, smurfing or layering — no real directional pressure on the book.",
             "trigger": "z_vol > 2 e |retorno| < 0,5%",
             "trigger_en": "z_vol > 2 and |return| < 0.5%"},
            {"id": "orderbook_depth", "name": "Profundidade anômala do book (roadmap)",
             "name_en": "Anomalous orderbook depth (roadmap)",
             ...
             "desc_en": "Requires an authenticated orderbook snapshot. Detects sudden walls and mass liquidity removal — a proxy for manipulation and front-running of illicit deposits.",
             "trigger": "variação de profundidade L10 > 3σ",
             "trigger_en": "L10 depth variation > 3σ"},
            {"id": "withdrawal_dispersion", "name": "Dispersão de saques on-chain (roadmap)",
             "name_en": "On-chain withdrawal dispersion (roadmap)",
             ...
             "desc_en": "Number of unique destinations for BTC/ETH/stablecoin withdrawals per day per exchange. High dispersion = layering. Requires on-chain VASP attribution (blockchain intelligence connectors).",
             "trigger": "Entropia de Shannon dos destinos > p95 12m",
             "trigger_en": "Shannon entropy of destinations > 12-month p95"},
            {"id": "pix_to_crypto_lag", "name": "Lag PIX→cripto (roadmap)",
             "name_en": "PIX→crypto lag (roadmap)",
             ...
             "desc_en": "Time between PIX deposit and first on-chain outflow per account. Lags < 10 min with high value indicate laundering automation.",
             "trigger": "lag < 600s e valor > R$ 50k",
             "trigger_en": "lag < 600s and value > R$ 50k"},
            {"id": "smurfing", "name": "Smurfing / estruturação",
             "name_en": "Smurfing / structuring",
             ...
             "desc_en": "Many small deposits/withdrawals below the usual screening thresholds. Detectable via daily trade count and average ticket size.",
             "trigger": "contagem +3σ com ticket médio < R$ 5k",
             "trigger_en": "count +3σ with average ticket < R$ 5k"},
            {"id": "ring_of_mules", "name": "Ring of mules (BR multi-venue)",
             "name_en": "Ring of mules (BR multi-venue)",
             ...
             "desc_en": "Day on which 3 or more BR series (Mercado Bitcoin + Foxbit + Binance BRL, any asset) simultaneously trigger Z≥2. Indicates a fleet of mules operating in parallel across multiple VASPs in the same event. NovaDAX, Bitso BR, BitPreço and Ripio Trade will be added automatically once their public klines are available.",
             "trigger": "≥3 séries BR com Z≥2 no mesmo dia",
             "trigger_en": "≥3 BR series with Z≥2 on the same day"},
            {"id": "ensemble", "name": "Ensemble severity (corroboração cruzada)",
             "name_en": "Ensemble severity (cross-corroboration)",
             ...
             "desc_en": "Aggregates independent detectors firing on the same (date, exchange). If 2+ distinct metrics corroborate, classifies as high; 3+ as critical. Inspired by external benchmark agent voting (Stablecoin + SingleExch + HourlyCorr = 3/8 = HIGH). Reduces false negatives from solo detectors in ambiguous regimes.",
             "trigger": "≥2 metrics distintos mesma (date,source) → high; ≥3 → critical",
             "trigger_en": "≥2 distinct metrics on same (date, source) → high; ≥3 → critical"},
            {"id": "hourly_corr", "name": "HourlyCorr (intraday cross-venue)",
             "name_en": "HourlyCorr (intraday cross-venue)",
             ...
             "desc_en": "Hourly granularity across Binance BRL + Mercado Bitcoin + Foxbit. For each (venue, pair), compares hourly volume against the 14-day median for the same hour-of-day. If 3+ distinct venues trigger ratio ≥3 in the same UTC hour, emits a cross-venue alert — catches pre-attack positioning that the daily baseline smooths out.",
             "trigger": "ratio ≥3 em ≥3 venues distintos na mesma (date, hour_utc); 4+ combos ou ratio ≥10 → critical",
             "trigger_en": "ratio ≥3 across ≥3 distinct venues on same (date, hour_utc); 4+ combos or ratio ≥10 → critical"},
            {"id": "tron_outflow", "name": "Saída de TRC-20 USDT (roadmap on-chain)",
             "name_en": "TRC-20 USDT outflow (on-chain roadmap)",
             ...
             "desc_en": "Daily sum of USDT outflows from exchange hot-wallets on the TRON network. Brazilian post-PIX cash-out is mostly routed via TRC-20. Cross-correlated with the local USDT-BRL peak within ±24h.",
             "trigger": "outflow Z≥2 e correlação com pico USDT-BRL ≥ 0.6",
             "trigger_en": "outflow Z≥2 and correlation with USDT-BRL peak ≥ 0.6"},
            {"id": "funding_rate", "name": "Funding rate de perpétuos",
             "name_en": "Perpetuals funding rate",
             ...
             "desc_en": "Strongly negative funding rate on BTC/ETH perpetuals = abnormal selling pressure (forced cash-out or defensive hedge after an incident).",
             "trigger": "rate ≤ −0,05% por 8h (high) ou ≤ −0,1% (critical)",
             "trigger_en": "rate ≤ −0.05% for 8h (high) or ≤ −0.1% (critical)"},
        ],
    }
